...
const Admin = () => {
  ...
  const handleEditModalShow = (id) => {
    ...
    if (key === 'games') {
      ...
      clickedItem = characterState.data?.filter(
  ...
  const handleEditInstance = (key, item) => {
    if (key === 'games') {
      axios.put(gameUrl + clickedItem.id, item).then(
        setTimeout(() => {
          gameState.service()
        }, 1000)
      )
    } else if (key === 'characters') {
      axios.put(characterUrl + clickedItem.id, item).then(
        setTimeout(() => {
  ...
  const handleDeleteInstance = () => {
    if (key === 'games') {
      axios.delete(gameUrl + clickedItem.id).then(
        setTimeout(() => {
          gameState.service()
        }, 1000)
      )
    } else if (key === 'characters') {
      axios.delete(characterUrl + clickedItem.id).then(
        setTimeout(() => {
          characterState.service()
        }, 1000)
      )
  ...
  const handleAddInstance = (tabKey, item) => {
    if (tabKey === 'games') {
      ...
      axios.post(characterUrl, item).then(
        setTimeout(() => {
          characterState.service()
        }, 1000)
      )
              ...
              <Table responsive striped bordered>
                ...
                <tbody>
                  {gameState.isFetching ? (
                    ...
                    gameState.data
                      ?.filter((game) =>
                        game.title.toLowerCase().includes(gamesQuery)
                      )
                      .map((data) => (
                        <tr key={data.id}>
                          <td>
                          ...
                          <td>
                            <ButtonGroup>
                              ...
                              <Button
                                variant='danger'
                                onClick={() => handleDeleteModalShow(data.id)}
                              >
                                <svg
                                  className='btn-icon'
                                  xmlns='http://www.w3.org/2000/svg'
              ...
              <Table responsive striped bordered>
                ...
                <tbody>
                  {characterState?.isFetching ? (
                    ...
                    characterState.data
                      ?.filter((character) =>
                        character.name.toLowerCase().includes(characterQuery)
                      )
                      .map((data) => (
                        <tr key={data.id}>
                          <td>
                            <OverlayTrigger
                              ...
                              overlay={
                                <Tooltip
                                  id={`tooltip-character-image-${data.id}`}
                                >
                                  <Image
                                    className='product-image-tooltip'
                                    src={data.image}
